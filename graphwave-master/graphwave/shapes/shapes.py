...
def cycle(start, len_cycle, role_start=0, plot=False):
    ...
    graph.add_weighted_edges_from([(start + 1, start + 2, 2.0)])
    graph.add_weighted_edges_from([(start + 2, start + 3, 3.0)])
    graph.add_weighted_edges_from([(start + 3, start    , 2.0)])
    graph.add_weighted_edges_from([(start    , start + 4, 1.0)])
    graph.add_weighted_edges_from([(start + 4, start + 5, 2.0)])
    graph.add_weighted_edges_from([(start + 5, start + 6, 3.0)])
    graph.add_weighted_edges_from([(start + 6, start    , 2.0)])
...
def house(start, role_start=0, plot=False):
    ...
    graph.add_weighted_edges_from([(start + 3, start    , 1.0)])
    graph.add_weighted_edges_from([(start    , start + 2, 1.0)])
    graph.add_weighted_edges_from([(start + 1, start + 3, 1.0)])
    graph.add_weighted_edges_from([(start + 4, start + 0, 1.0)])
    graph.add_weighted_edges_from([(start + 4, start + 1, 1.0)])
